...
Item {
    ...
    function saveHistory() {
        const json = JSON.stringify(root.messages, null, 2).replace(/'/g, "'\\''")
        const dir = getHistoryPath().replace(/\/history\.json$/, "")
        const p = _writeFile.createObject(root)
        p.command = ["bash", "-c", "mkdir -p '" + dir + "' && printf '%s' '" + json + "' > '" + getHistoryPath() + "'"]
        p.running = true
    }
    ...
    Component { id: _writeFile; Process {} }
